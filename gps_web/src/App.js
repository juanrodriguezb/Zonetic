...
var marks = [
  {
    value: this.state.date_in.getTime(),
    label: this.state.date_in.getTime().toString,
  },
  {
    value: this.state.date_fin.getTime(),
    label: this.state.date_fin.getTime().toString,
  },
];
...
class App extends Component {
  constructor() {
    ...
    this.state = {
      ...
      coordt: { latt: 10.9878, lngt: -74.7889 },
      lat: 8.75,
      lng: -75.883,
  ...
  callAPI_users() {
    axios.get(API_URL_3).then((res) => {
      for (var i = 0; i < res.data.length; i++) {
        this.state.Opt.push({
          value: res.data[i].truck,
          label: "Camion ".concat(res.data[i].truck.toString()),
        });
      }
    });
  }
    ...
    axios
      ...
      .then((res) => {
        for (var t = 0; t < res.data.length; t++) {
          this.state.Tmp.push({ value: res.data[t].truck });
          this.state.Tmp2.push({ value: ((res2.data[t]).getTime()).truck, label: (((res2.data[t]).getTime()).truck).toString()});
        }
      });
  }
  ...
  render() {
    return (
      <div>
        <title>
          GPS
          <span role="img" aria-label="Satellite Antenna">
            📡
          </span>
        </title>
        ...
        <div>
          ...
          <h4 style={{ top: "20.5%", left: "2%" }}>
            <body>
              {"Timestamp: " +
                (
                  new Date(parseFloat(this.state.coord_text.time, 10)) + ""
                ).split("(")[0]}
            </body>
          </h4>
        </div>
